...
def assign_work_category(df):
    ...
    construction = ['Contractor','Renovations','Builder','Paint','Trucking','Hauling','Demolition','Fence','Fencing','Grading','Paving','Pave','Concrete','Roof','Electrical','Install','Installation','Plumbing','Restoration','Windows','Door Replacement','Asbestos','Abatement','Heating','Air','Resurface','Drainage','Masonry','Flooring']
    construction_1 = ['Contractors','Contracting','Builders','Builder','Painting','Remodeling','Roofing','Electric','Energy','Floors','Floor','Construction','Building']
    ane=['Engineer','Architect','Environmental','Survey','Design','Erosions','Inspections','Archeologist','Infrastructure','Aerospace']
    ane_1=['Engineering','Surveying','Mapping','Planning']
    p_services=['Health','Audit','Medical','Consultants','Lawyer','Law','Legal','Bank','Account','Psychology','Financial','Counseling','Business Analyst','Pharmacy','Pharmaceuticals']
    p_services_1=['Tax','Cpa']
    o_services=['Childcare','Repair','Towing','Solution','Research','Service','Maintenance','Restoration','Restaurant','Enforcement','Parking','Sales','Technology','Management','Lawn','Landscaping','Landscape','Data','Staffing','Cleaning','Transportation','Janitor','Waste','Pest','Associates','Advertising','Real Estate','Printing','Security','Marketing','Graphic Design','Interior Design','Training','Software Maintenance','Insurance','Delivery']
    o_services_1=['Child Care','Daycare','Preschool','Creative','Solutions','Technologies','Lawncare','Landscapes','Learning','Photography','Graphics','Graphic','Imaging','Media','Systems','Fire','Protection']
    goods=['Good','Parts','Equipment','Supply','Break','Car','Truck','Tire','Hardware','Product','Rental','Sheet Metal Fabrication','Lease','Software','License']
...
def get_number_of_instances(df,drop_dups,primary_key,two_cols):
...
def data_cleaner(df,df_name,messy_column_name,new_column):
    for i in df[messy_column_name].unique():
        print(df_name+'.loc['+df_name+'[\''+messy_column_name+'\']==\''  +str(i)+'\',\''+new_column+'\'] = \'\'')
